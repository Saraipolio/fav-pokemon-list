...
struct PokemonView: View {
    ...
    var body: some View {
        VStack {
            Image(pokemon.type).resizable().padding().frame(height: 400.0)
            Image(pokemon.imageName).resizable().padding().frame(height: 350.0).background(Color.white).clipShape(Circle()).shadow(radius: 10).offset(x:0,y:-160).padding(.bottom,-150)
